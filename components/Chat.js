...
const Chat = () => {
    ...
    const [msgList, setMsgList] = useRecoilState(messagesList);

    const sendMessage = (e, from = "") => {
        if (message.trim() !== "" && ((from === "key" && e.key === "Enter") || from === "button")) {
            e.preventDefault();
            setMsgList([...msgList, {
                msg: message,
        ...
        <div className="w-full">
            {Object.keys(selectedChat).length === 0 ? (<StartChat />) : (
                <div className="flex flex-col">
                    ...
                    <form className="flex items-center justify-center my-4 mx-3">
                        <div className="flex items-center justify-center border border-[#dbdbdb] rounded-3xl w-full mx-5">
                            ...
                            <textarea
                                ref={msgInput}
                                className="flex-1 resize-none border-none overflow-auto px-2 py-[9px] 
                                appearance-none h-10 leading-[18px] focus:ring-0 justify-center p-3"
                                value={message}
                            ...
                            {!!message && <button onClick={e => sendMessage(e, "button")} className="mr-3 font-semibold text-[#0095f6]">Send</button>}
                        </div>
                    </form>
                </div>
            )}
